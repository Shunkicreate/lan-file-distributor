...
func (s *FileService) GetFile(path string, width uint, height uint) (image.Image, error) {
	return s.repo.GetFile(path, width, height)
}

func (s *FileService) GetFiles(paths []string, width uint, height uint) ([]image.Image, error) {
...
func (s *FileService) GetRandomFiles(folder string, count int, width uint, height uint) ([]image.Image, error) {
